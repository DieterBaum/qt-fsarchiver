...
int createar_item_stdattr(csavear *save, char *root, char *relpath, struct stat64 *statbuf, cdico *d, int *objtype, u64 *filecost)
{
    ...
    *filecost=FSA_COST_PER_FILE; // fixed cost per file
    ...
    if (meldeflag  == 0) {
           // Anzeige im Terminal 
            printf ("A total of  %.5lld  directories or files must be backed up.\r", (long long)save->objectid);
    	...
    	if (meldeflag  == 1 ) { 
           ...
           anzahlfile1 = save->objectid; 
           werte_uebergeben(anzahlfile1,3); 
       }
